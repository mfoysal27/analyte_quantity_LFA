...
% hsv=rgb2hsv(I);
% s = hsv(:,:,2);
s=double(I(:, :, 1)>1.1.*I(:, :, 2));
imshow(s)

% s_mask=im2bw(s, 0.1);
% % imshow(s_mask);
foreground = s >0 % Or whatever.imshow(I(:, :, 2)<170)
% foreground = bwconvhull(foreground, 'objects');
foreground = bwareaopen(foreground, 10); % or whatever.
% foreground= watershed(foreground);
imshow(foreground, []);
...
for i=1:length(measurements)
    bb = measurements(i).BoundingBox;
imshow(bb, []);
croppedImage = imcrop(I, bb);
% rect=(poly2mask(bb(1), bb(2), bb(3), bb(4)));
% croppedImage=createMask(bb, I);
figure(i)
imshow(croppedImage);
% surf(croppedImage(:, :, 1));
hold on
